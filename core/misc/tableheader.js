...
/**
 * 28 jan 16
 * @return stricky header at people permission page  for IE browser
 */
jQuery('document').ready(function(){
  jQuery('form#user-admin-permissions table.sticky-header').css('visibility', 'hidden');
});
 jQuery(window).scroll(function() { 
  var eTop = jQuery('form#user-admin-permissions table#permissions').offset().top; 
 var a = eTop - jQuery(window).scrollTop();
  if(a <= 42)
     {
      jQuery('form#user-admin-permissions table.sticky-header').css('visibility', 'visible');
     }
});